...
function LoadMoreFeeConfig(type) {
    ...
    if (type == 'reset') {
        FeeManagerPage  = 1;
        $('#fee_config_container').html("");
        $("#fee_config_container").parent().hide();
    }
    ...
    $.ajax({
		...
		window.location = jsVars.FULL_URL+jsVars.LOGOUT_PATH;
            ...
            if(responseObject.status==1){
                ...
                $('#fee_config_container').parent().removeClass('hide');
                $('#fee_config_container').append(data);
                ...
                $("#load_msg_div").hide();
                $("#fee_config_container").parent().show();
...
function changeStatusUser(params) {
    ...
    $.ajax({
        url: '/fee-configs/changeStatus/'+params,
        type: 'get',
        dataType: 'json',
        headers: {
            "X-CSRF-Token": jsVars._csrfToken
