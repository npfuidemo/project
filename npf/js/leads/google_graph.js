       ...
       $('.graph-footer-msg').remove();
        ...
        google.charts.load('current', {
            ...
            var linearOptions = {
                //title: json['title'],
                titlePosition: 'none',
                tooltip: {isHtml: true, textStyle: {fontSize: 7 }},
                legend: 'none',
...
function makeBarGraph(json) {
	...
	if(typeof json['height'] != 'undefined') {
        ...
        var isRedirectLink = (json['id_container'] === "stage-wise-chart-graph");
        if(isRedirectLink) {
            var redirectLinkIndex = json['content'][0].findIndex(rLI => rLI === "redirectLink");
                if(redirectLinkIndex > 0) {
                    for (var i = 0, l = json['content'].length; i < l; i++) {
                        if(i > 0) {
                            redirectLinkArray.push(json['content'][i][json['content'][i].length - 1]);
                        }
                        json['content'][i].splice(redirectLinkIndex,1);
                    }
                }
        }
    ...
    google.charts.load('current', {
        ...
        var options = {
			...
			chartArea: {left:'16%', top: '8%', width: "75%", height: "70%" },
            vAxis: {
				title: json['vAxis_title'], 
				viewWindow:{min:0},
				titleTextStyle: {
...
//                        $('#city-wise-chart-graph').html('');
                    ...
                    var range_value = json['range_value'];
                    ...
                    $.ajax({
                        ...
                        type: 'post',
                        dataType: 'json',
                        data: {'college_id': college_id, 'range_value': range_value, 'type':type, 'clickingvalue':clickingvalue, 'selectedval':selectedval},
    ...
    if(typeof id != 'undefinded'){
        switch (id) {
            case 'country-wise-download':
                options.pageWidth = '8.5in';
                options.pageHeight = '16in';
                break;
            default:
...
function makeScrollable(){
    var tabLocation = $('#data-tab-container').offset().top - 125;
...
function makeLineCharts(json){
      google.charts.load('current', {
        ...
        chart: {
		  ...
		  width: '100%',
		  height: 350,
		chartArea: { left: '8%', top: '8%', width: "90%", height: "70%" },
        };
...
function makeLineChartsOld(json){
      /*google.charts.load('current', {'packages':['line']});
      google.charts.setOnLoadCallback(drawLineCharts,json);*/
    google.charts.load('current', {
        callback: function () {
        // Create the data table.
        ...
        var data = new google.visualization.DataTable();
            ...
            */
            ...
            var chart = new google.charts.Line(document.getElementById(json['id_container']));
...
function drawMatarialBarChart(json) {
    ...
    google.charts.load('current', {
        ...
        if(maxVaxis <= 100 ){
           vAxis = {format: 'decimal',maxValue: maxVaxis + maxVaxis/3, textStyle: { fontSize: '12', bold: true,}};
        }
        ...
        var newlegend = { position: 'top', maxLines: 3, alignment:'center', textStyle: {fontSize: 12} };
        if(json['legend'] === 'none'){
        ...
        var options = {
            ...
            bars: 'vertical',
            vAxis: vAxis,
			hAxis: {title: '', textStyle: { fontSize: '12', bold: true,}},
            height: 380,
