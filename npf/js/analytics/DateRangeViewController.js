...
$(document).ready(function () {
    ...
    var getBaseDates = function () {
		// 6 - Today
		var todayDate = [moment().subtract(0, 'days'), moment()];
        // 7 - Yesterday
        var yesterdayDate = [moment().subtract(1, 'days'), moment().subtract(1, 'days')];
        // 0 - last 7 days
        ...
        var baseRanges = [
			...
			{label: 'Today', range: todayDate, id: 6},
            {label: 'Yesterday', range: yesterdayDate, id: 7}
        ];
    ...
    var getPreviousDates = function () {
		...
		// 6 - Today
        var previousPeriodTodayRange = [moment().subtract(1, 'days'), moment().subtract(1, 'days')];
		
		// 6 - Yesterday
		var previousPeriodYesterdayRange = [moment().subtract(2, 'days'), moment().subtract(2, 'days')];
        ...
        var previousRanges = [
			...
			{label: 'Previous Today', range: previousPeriodTodayRange, id: 6},
			{label: 'Previous Yesterday', range: previousPeriodYesterdayRange, id: 7},
        ];
    ...
    var init = function () {
        ...
        if ((document.domain === "localhost") && bTesting) {
    ...
    var setMode = function (bBaseMode, dates, rangeSelectIds) {
        ...
        if (bBaseMode) {
            ...
            if (compareSelectId === "2")
                compareSelectId = "0"; // default
        ...
        $(".oneDateSelect").on("change", function (e) {
            ...
            switch (idNum) {
                ...
                case 0 :
                    ...
                    break;
                case 1 :
                    day = moment().subtract(7, 'days').format("DD/MM/YYYY");
                    break;
                case 2 :
                    day = moment().subtract(2, 'weeks').format("DD/MM/YYYY");
                    break;
                case 3 	:
                    day = moment().subtract(1, 'months').format("DD/MM/YYYY");
                    break;
                case 4 :
                    day = "";
                    break;
    ...
    var reset = function () {
		...
		$(".baseSelect select").val('8').trigger('change');
    }
    ...
    $(".baseSelect").on("change", function (e) {
        ...
        if (idNum === 8) {
            ...
            $(".validationMessage").text("Please enter 2 dates for your Base range.");
    ...
    var setCurrentFocus = function (inputId) {
        ...
        if (typeof currentFocusId != "undefined" && currentFocusId.indexOf("Compare") !== -1)
    ...
    var setBaseDate = function (startDate, endDate, posStr) {
        ...
        if (posStr === "End")
            console.log("setBaseDate baseDate: ", baseDate);
        if (posStr === "End")
    ...
    var clearCompareDatesCustom = function () {
        $(".inputCompareStartDate").val("");
        $(".inputCompareEndDate").val("");
    ...
    var setCompareDate = function (startDate, endDate, posStr) {
        ...
        if (null === compareStartMoment)
            compareStartMoment = moment(); //getDateMoment("CompareStart");
        if (null === compareEndMoment)
            compareEndMoment = moment(); ///getDateMoment("CompareEnd");
    ...
    var setBaseRangeMessage = function (dates) {
        ...
        if (bCompareChecked && null === compareStartMoment) {
            ...
            var customJoinStr = "<span class='compareRangeTo'> - To -  </span>";
            var customFullStr = customAStr + customJoinStr + baseRangeStr;

            $(".baseRange1").html(customFullStr);
    ...
    var baseStartCaseHandler = function (startDate) {
        ...
        if (bValidBaseFrom) {
			...
			// Set End Date for single date
			baseEndCaseHandler(startDate);
        } else {
    ...
    var validateStartBeforeEnd = function (startDateMoment, endDateMoment, bValidateLog, logMessage) {
        ...
        if (yearsDiff >= 1) {
            ...
            if (bValidateLog)
                console.log(yearsDiff, 'years diff - all good');
    ...
    var onCalendarsChange = function (dates, el) {
		...
		var bBaseFocus = (typeof currentFocusId != "undefined" && currentFocusId.indexOf("Base") !== -1);  
    ...
    $(".applyDates").on("click", function () {
        ...
        if (null !== compareStartDate)
            compareStartDate.setHours(0, 0, 0);
        if (null !== compareEndDate)
            compareEndDate.setHours(23, 59, 59);
    ...
    /*if(! $(".date-range").closest(".open").length ) {
     //console.log("has no parent with the class .open");
     alert('hello');
     }
    ...
    $('.date-range').click(function () {
        //var xyz1 = event.target.id;
        if ($(this).siblings('.dd-range')) {
            //$('.datepicker-calendar').DatePickerClear();
            //$(".baseSelect select").val('2').trigger('change');
            //setCalendarsByCurBaseRange();
            /*if($(".inputCompareCheckbox").prop('checked') == true){
             clearCompareDatesCustom();
             }*/
           reset();
        }
    });
	$('.noCompare').click(function () {
		//alert('asdfg');
		jQuery(this).siblings().find(".compareRowContainer").addClass('hideCompare');
	})
