$(document).ready(function(){
    loadMoreDashboardLeadsData();
});

function loadMoreDashboardLeadsData(){
    
    var data = $('#FilterForm').serializeArray();
    var collegeId = $("#college_id").val();
    var dateRange = $('#date_range_productivity').val();
    $.ajax({
        url: jsVars.FULL_URL + '/reports/ajax-leads-dashboard-list',
        type: 'post',
        dataType: 'html',
        data: {
            "data": data,
        ...
        success: function (response) { 
            ...
            if(responseObject.status==1){
                if(responseObject.data.html) {
                    ...
                    $('#load_more_results').append("<tr><td colspan='8'><div class='text-danger text-center fw-500'>No More Records</div></td></tr>");
