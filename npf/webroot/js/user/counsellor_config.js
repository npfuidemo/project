...
function createInputApplicationCounsellorConfig(currentObj, selected, typeselected,call_type) {
    ...
    if (selected == '' && (html_field_id == 'ud|career_utsav_area_id' )) {
        /// load value from 
        var selectedEvents = $("[name='" + cur_val_name + "']").parents('div.block_first').find('.career_utsav_id').last().val();
    ...
    if (selected == '' && (html_field_id == 'ud|seminar_preference_id' || html_field_id == 'ud|mock_preference_id')) {
        /// load value from 
        var selectedEvents = $("[name='" + cur_val_name + "']").parents('div.block_first').find('.career_utsav_id').last().val();
        var selectedUtsavArea = $("[name='" + cur_val_name + "']").parents('div.block_first').find('.career_utsav_area_id').last().val();
    ...
    if (selected == '' && ('campu|publisher_id' == html_field_id || 'pub|publisher_id' == html_field_id )) {
        ...
        /// load value from 
        var channel_val = $("[name='" + cur_val_name + "']").parents('div.block_first').find('.campu\\|traffic_channel').last().val();
    ...
    if (selected == '' && 'campu|name_value' == html_field_id) {
        ...
        if(medium_val_arr !== null && typeof medium_val_arr !='undefined' && medium_val_arr!=''){
...
function getReferrerListConfig(elem, select_name_value, class_name,tfc) 
{
    ...
    $.ajax({
        ...
        async: false,
        data: {'college_id': college_id, 'traffic_channel': tfc,'registration_instance':registration_instance,'includetc':1},
        headers: {'X-CSRF-TOKEN': jsVars._csrfToken},
        success: function (json) {
            var responseObject = $.parseJSON(json);
            if(responseObject.status==1){
        ...
        if (typeof elemClass != 'undefined' && elemClass.indexOf('field_') > -1) {
            ...
            if (['1','2','6','7','8','9','10'].indexOf($(this).val())>-1) {//campaign option
             ...
             $.each($(this).val(), function(k,pub_value){
                ...
                //conversion source
                if ($(currentElementObj).parents('.block_first').find('.campap\\|conversion_source_value').length > 0) {
...
$(document).on('change', "select.campu\\|publisher_id.sel_value,select.publisher_filter.sel_value", function () {
    ...
    if(pub_value !==null ){
        $.each(pub_value, function(k,pub_val_single){
            ...
            var tfc     = pub_val_ar[1];
            getCompaignsSourceConfig(currentObject, pub_val, sel_svalue,'',tfc);
            if($(currentObject).parents('.block_first').find('.campap\\|conversion_source_value').length>0){
        ...
        success: function (json) {
            if (json['redirect']) {
                ...
                for(var field in json['errorList']) {
                    console.log('#' + Section + ' #collapseTelephony' + collegeId + ' div.' + field);
