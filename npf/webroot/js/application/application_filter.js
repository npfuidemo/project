...
function removeApplCondition(elem) {
    ...
    var cond_div = $(elem).closest('.block-repeat');
    var select_class = $(elem).closest('.condition_div').find('.form-group.chosen-select.sel_field');
    var deletedVal = select_class.val();
    var deletedField = deletedVal.split("||");
    if(deletedField[0]=='fde|application_sub_stage'){
        $("select.sel_value.application_stage",cond_div).prop("multiple",'multiple');
        $(".sel_value.application_stage",cond_div)[0].sumo.reload(); 
    }
...
function createApplicationFilterConfig(currentObj, selected, typeselected,call_type) {
    ...
    if (arr.length > 2) {
        ...
        if(html_field_id === 'upf|order_no'){
            $(currentObj).parent().parent().parent().find('.dependent_info').html('This field is dependent on applicant school preference');
        }
        ...
        if (type === "dropdown" || type === "predefined_dropdown") {
            ...
            } else if (html_field_id ===  'fd|application_followup_date' ) {
                html = "<select placeholder='Select an Option' class='sumo_select application_followup_date sel_value' name='" + cur_val_name + "' data-field='"+html_field_id+"'>";
            ...
            }else if(html_field_id === 'psal|id' || html_field_id === 'upf|order_no') {
                html = "<select  placeholder='Select an Option' 'display_selected_options'=false class='chosen-select sel_value " + html_field_id + "' name='" + cur_val_name + "' data-field='"+html_field_id+"' data-ct='"+call_type+"'>";
                if(html_field_id === 'upf|order_no') {
                    html += '<option value="">Select Order</option>';
                }
            }else if(html_field_id === 'fd|application_stage') {
                var classval = html_field_id.split("|");
                var closestDiv = $(currentObj).closest('.block-repeat');
                if($(".sel_value.fde\\|application_sub_stage",closestDiv).length > 0){
                    html = "<select  placeholder='Select an Option' class='sumo_select sel_value " + classval[1] + "' name='" + cur_val_name + "' data-field='"+html_field_id+"' data-ct='"+call_type+"'>";
                }else{
                    html = "<select multiple='multiple' placeholder='Select an Option' class='sumo_select sel_value " + classval[1] + "' name='" + cur_val_name + "' data-field='"+html_field_id+"' data-ct='"+call_type+"'>";
                }
            }
            ...
            if(html_field_id === 'fde|application_sub_stage' && selected === ''){
                    ...
                    var stageId =  $(".sel_value."+parentFieldId,currentDiv).val();
            ...
            if(html_field_id === 'ud|lead_sub_stage' && selected === ''){
                    ...
                    var stageId =  $(".sel_value."+parentFieldId,currentDiv).val();
                    if(stageId!= 0 && typeof stageId != 'undefined'){
                      getLeadSubstage(stageId,$(".sel_value."+parentFieldId,currentDiv),[]);
                    }
            }
...
//            if (jQuery("[name='" + cur_type_name + "']").val() === 'between' && (inputId !== "application_followup_date")) {
//                class_date = "date_time_rangepicker_report";
//            }if (inputId == 'last_application_followup_complete_date') {
...
//    if (selected === '' && $.inArray(html_field_id,['campu|source_value','campap|conversion_source_value']) >= 0 ) {
    if ($.inArray(html_field_id,['campu|source_value','campap|conversion_source_value']) >= 0 ) {
//        
...
//    if (selected === '' && $.inArray(html_field_id,['campu|medium_value','campap|conversion_medium_value']) >= 0) {
    if ($.inArray(html_field_id,['campu|medium_value','campap|conversion_medium_value']) >= 0) {
...
//    if (selected === '' && $.inArray(html_field_id,['campu|name_value','campap|conversion_name_value']) >= 0) {
    if ($.inArray(html_field_id,['campu|name_value','campap|conversion_name_value']) >= 0) {
        /// load value from 
    ...
    if (html_field_id === 'CreatedBySelect'){
        if (typeof CollegeWiseCreaterList !== "undefined" && typeof CollegeWiseCreaterList['CollegeWise'] !== "undefined" && $('#FilterApplicationForms select#college_id').val() in CollegeWiseCreaterList['CollegeWise'])
        {
...
//        if(jQuery("[name='" + cur_val_name + "']").hasClass("date_time_rangepicker_dynamic"))
        ...
        {
            if(jQuery("[name='" + cur_val_name + "']").data('DateTimePicker')){
...
//            jQuery("[name='" + cur_val_name + "']").removeClass('datetimepicker_report').addClass('date_time_rangepicker_report');
            jQuery("[name='" + cur_val_name + "']").removeClass('datetimepicker_report').addClass('date_time_rangepicker_dynamic');
        ...
        jQuery("[name='" + cur_val_name + "']").removeClass('date_time_rangepicker_report date_time_rangepicker_dynamic date_time_rangepicker_dynamic_followup');

        jQuery("[name='" + cur_val_name + "']").val('');
        jQuery("[name='" + cur_val_name + "']").addClass('datetimepicker_report');
        ...
        jQuery("[name='" + cur_val_name + "']").removeClass('datepicker_report daterangepicker_report date_time_rangepicker_dynamic date_time_rangepicker_dynamic_followup');
        jQuery("[name='" + cur_val_name + "']").datepicker('remove');
    ...
    var dateRangePickerElem = $("[name='" + cur_val_name + "']").hasClass('date_time_rangepicker_report');
    var datePickerElem     = $("[name='" + cur_val_name + "']").hasClass('datetimepicker_report');
...
//            loadSavedAliases(currentElementObj,'district_id');
        ...
        else if (typeof elemClass != 'undefined' && elemClass.indexOf("fde|application_sub_stage") > -1) {
            var parentFieldId = 'application_stage';
...
//            loadSavedAliases(currentElementObj,'city_id');
            ...
            var stageId =  $(".sel_value."+parentFieldId).val();
            if(stageId!= 0 && typeof stageId != 'undefined'){
                getLeadSubstage(stageId,$(".sel_value."+parentFieldId),select_name_value);
            }
        }
        else if (typeof elemClass != 'undefined' && elemClass.indexOf('') > -1) {
...
function applicationAndOrBlockCondition(elem) {
    $('#FilterApplicationForms .block_criteria a.btn_or,#FilterApplicationForms .block_criteria a.btn_and').removeClass('active');
    if ($(elem).hasClass('btn_or')) {
        $('#FilterApplicationForms .block_criteria a.btn_or').addClass('active');
        $('#app_block_condition').val('or').trigger('change');
    } else if ($(elem).hasClass('btn_and')) {
        $('#FilterApplicationForms .block_criteria a.btn_and').addClass('active');
...
//            'eq_like': 'Exactly Matches'
    ...
    if(selectedField.indexOf('fd|application_followup_date||dropdown') === 0){
        operator['dropdown'] =  {
                                    'eq': 'Equals'
                                };
    }
    if(selectedField.indexOf('fd|last_application_followup_complete_date||date||') === 0){
        operator['date'] =  {
                                    'between': 'Between'
                                };
    }
    if(selectedField.indexOf('psal|id||') === 0 ){
        operator['dropdown'] =  {
                                    'eq': 'Equals',
                                };
    }
    if(selectedField.indexOf('rfl|status||') === 0 ){
        operator['dropdown'] =  {
                                    'eq': 'Equals',
                                };
    }
...
function checkDependentField(field, currentObj, type, selected){
    ...
    field_data['applicable_for']  = 'onlyCrop';
    jQuery.ajax({
...
function addMoreBlockConditionAjax() {
    ...
    var college_id  = jQuery('#FilterApplicationForms #college_id').val();
    var form_id = $("#FilterApplicationForms #form_id").val();
    ...
    $.ajax({
        url: '/applications/load-single-block' ,
        type: 'post',
...
function getAdvanceFilterOptions(optionArray,callFrom){
    ...
    if(optionArray){
        $.ajax({
            ...
            dataType: 'html',
            async:asyncVal,
            data: {'index':0,'optionArray':optionArray},
...
//    $('#FilterApplicationForms #form_id').trigger('change');
...
function renderSavedFilter(valueObj, module) {
    ...
    $.ajax({
        url: '/applications/render-saved-filter' ,
        type: 'post',
        data: {form_id: form_id, 'college_id': college_id,'valueObj':valueObj},
        dataType: 'html',
        async : asyncVar,
        headers: {
            "X-CSRF-Token": jsVars._csrfToken
...
function UpdateCreatedBySelect(CreatedByList,CreatedByIdSelected)
{
    ...
    $('#FilterApplicationForms .CreatedBySelect option[value=""]').remove(); 
...
function tourAF(){
	var tourAdvanceFilter = new Tour({
        steps: [
            {
                element: ".expended-search-link",
                title: "Search <sup class='text-danger'>New</sup>",
                content: "Search on the basis of selected criteria",
            ...
            },
            {
                ...
                content: "Search application that satisfy one or multiple conditions.",
            ...
            },
            {
                ...
                content: "Add a single application or upload applications in bulk.",
                template: "<div class='popover tour'><div class='arrow'></div><h3 class='popover-title'></h3><div class='popover-content'></div><nav class='popover-navigation'><button class='btn btn-xs btn-npf btn-prev' data-role='prev'>Prev</button><button class='btn btn-xs btn-npf btn-next pull-right' data-role='next'>Next</button></nav></div>",
                placement: "left", 
            },
            {
	...
	// Initialize the tour
	tourAdvanceFilter.init();
	// Start the tour
	tourAdvanceFilter.start();
}
        ...
        if(i === 0){
            ...
            $('.condition_div').each(function(i,val){
                ...
                if(fieldName.indexOf("councellor_id||") !== -1 && (singleCounsellorSelected === '' || singleCounsellorSelected === '1')){
                    var fieldType = $(this).find('.sel_type').val();
                    var fieldValue = $(this).find('.sel_value').val();
                    if(fieldType === 'eq' && fieldValue.length ===  1 && singleCounsellorSelected === ''){
...
$(document).on('change', ".application_stage.sel_value", function () {
    ...
    var currentElementObj = $(this);
    if($(".sel_value.fde\\|application_sub_stage").length > 0){
...
$(document).on('change', ".lead_stage.sel_value", function () {
    ...
    if($(".sel_value.ud\\|lead_sub_stage").length > 0){
        getLeadSubstage(selectedStage,currentElementObj,[]);
    }
});

function getLeadSubstage(stage, currentElementobj,select_name_value) {
    class_name = 'ud\\|lead_sub_stage';
    var collegeId = jQuery(currentElementobj).parents('form').find('[name="s_college_id"]').val();
    if(stage==0){
...
function getSubstage(stage, currentElementobj,select_name_value) {
    class_name = 'fde\\|application_sub_stage';
    var form_id = jQuery(currentElementobj).parents('form').find('[name="form_id"]').val();
    if(stage==0){
        $(currentElementobj).parents('.block-repeat').find('.' + class_name+' option').remove(); 
        //$(currentElementobj).parents('.block-repeat').find('.' + class_name)[0].sumo.reload();  
        return;
...
function checkDocFilterType(currentObj){
    var value_field = $(currentObj).val();
    var doc_download = 0;
    if(value_field.indexOf('doc|') === 0 ){
        $('.sel_field').each(function(){
             if($(this).val().indexOf('doc|') === 0){
                doc_download = doc_download+1;
             }
        });
    }
    if(doc_download>1){
        $(currentObj).val('');
        $('.chosen-select').trigger('chosen:updated');
        var error_text = '<small class="text-danger">A document download field is already selected.</small>';
        $(currentObj).parent().parent().parent().find('.dependent_info').html(error_text);
        return false;
    }
}