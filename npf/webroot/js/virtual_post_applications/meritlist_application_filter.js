...
function confirmDeleteMeritlist(elem, type) {
    ...
    $('#ConfirmPopupArea').modal({backdrop: 'static', keyboard: false})
             .off('click', '#confirmYes').one('click', '#confirmYes', function (e) {
                e.preventDefault();
                    ...
                    removeApplConditionMeritList(elem);

                //is_filter_button_pressed = 0;
                var limit_counter     = parseInt($('#limit_counter').val()) ;
                limit_counter--;
                $('#limit_counter').val(limit_counter);
                $('#ConfirmPopupArea').modal('hide');
            });
    return false;
}
function removeApplConditionMeritList(elem) {
//    var div_class = 'form0' ;
//    var cond_block = jQuery(elem).parents('.block-repeat');
//    var cond_div = $(elem).closest('.block-repeat');
//    var select_class = $(elem).closest('.condition_div').find('.form-group.chosen-select.sel_field');
...
//        $(".sel_value.application_stage",cond_div)[0].sumo.reload();
//    }
    $(elem).closest('.condition_div_repeat').remove();
...
//    if (selected === '' && $.inArray(html_field_id,['campu|name_value','campap|conversion_name_value']) >= 0) {
    ...
    if (html_field_id === 'CreatedBySelect'){
        if (typeof CollegeWiseCreaterList !== "undefined" && typeof CollegeWiseCreaterList['CollegeWise'] !== "undefined" && jsVars.collegeId in CollegeWiseCreaterList['CollegeWise'])
        {
            UpdateCreatedBySelect(CollegeWiseCreaterList['CollegeWise'][jsVars.collegeId], postedCreatedBy, cur_val_name);
        }
    }
...
function checkConditionType(elem, module) {
    ...
    if (val === 'between') {
        ...
        if($("[name='" + cur_val_name + "']")[0] && $("[name='" + cur_val_name + "']")[0].sumo){
            ...
            $("[name='" + cur_val_name + "'] option[value='0']").remove();
            $("[name='" + cur_val_name + "']")[0].sumo.reload();
        }
    }
...
function applicationAndOrBlockCondition(elem) {
    $('.block_criteria a.btn_or,.block_criteria a.btn_and').removeClass('active');
    if ($(elem).hasClass('btn_or')) {
        $('.block_criteria a.btn_or').addClass('active');
        $('#app_block_condition').val('or').trigger('change');
    } else if ($(elem).hasClass('btn_and')) {
        $('.block_criteria a.btn_and').addClass('active');
...
function getSelectedChildTaxonomy(selectedParentId, childType){
    ...
    var html = '';
    var college_id = jsVars.collegeId;
...
function addMoreBlockConditionAjax() {
    ...
    $.ajax({
        url: '/applications/loadMeritListSingleBlock' ,
        type: 'post',
  ...
  function addMoreMeritListBlockConditionAjax() {
    ...
    if(limit_counter>=10)
    {
        alertPopup("Maximum Shortlisting Criteria Limit Reached.", 'error');
        //alert("Limit should not be greater than 5");
        return false;
    }
    $.ajax({
        url: '/applications/loadMeritListLogicBlock' ,
        type: 'post',
        data: {form_id: form_id, 'college_id': college_id},
        dataType: 'html',
        headers: {
            "X-CSRF-Token": jsVars._csrfToken
        },
...
function getAdvanceFilterOptions(optionArray){
    var asyncVal = true;
    if(optionArray){
        $.ajax({
            url: '/applications/loadMeritListSingleBlock',
 ...
 * @returns {Boolean}
 */
function filterColumnOptionsCollegeApplication(college_id,form_id){
    ...
    $.ajax({
        ...
        headers: {'X-CSRF-TOKEN': jsVars.csrfToken},
        beforeSend: function () {
            $(document).find('#save_merit_list').attr('disabled', true);
            showFilterLoader();
        },
        ...
        success: function (json) {
            if (typeof json['redirect'] !== 'undefined' && json['redirect'] !== "") {
               ...
               /****filter**********/
                ...
                getAdvanceFilterOptions(myJSON);
...
function renderSavedFilter() {
    var meritlist_id  = jQuery('#meritListId').val();
//    if(typeof meritlist_id === 'undefined' || meritlist_id==='0') {
//        return;
//    }
    $.ajax({
        url: '/applications/renderSavedMeritListFilter' ,
        type: 'post',
        data: {form_id: jsVars.formId, 'college_id': jsVars.collegeId, 'meritlist_id':meritlist_id},
        dataType: 'html',
        async : true,
        headers: {
            "X-CSRF-Token": jsVars._csrfToken
        ...
        success: function (Html) {
            ...
            $(document).find('#save_merit_list').attr('disabled', false);
            hideLoader();
        },
        error: function (xhr, ajaxOptions, thrownError) {
            hideFilterLoader();
...
function validateMeritListFilter() {
    validAdvanceFilter = true;
    var data = $('#meritlist_basic_settings_form').serializeArray();
    $.ajax({
