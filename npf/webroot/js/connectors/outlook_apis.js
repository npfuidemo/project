...
function disconnectOutlookApisAccount(hashed_data) {
    $('.modal').modal('hide');
    $('#ConfirmMsgBody').html('Are you sure you want to disconnect with Outlook Account? This action will delete all mapping data.');
    $('#ConfirmPopupArea').modal({backdrop: 'static', keyboard: false})
            .one('click', '#confirmYes', function (e) {
                e.preventDefault();
                ajaxDisconnectOutlookApisAccount(hashed_data);
            });
}

function ajaxDisconnectOutlookApisAccount(hashed_data) {
    $('.modal').modal('hide');
    $.ajax({
        url: '/connectors/outlook-apis/disconnect-outlook-apis-account',
        type: 'post',
        dataType: 'json',
...
function ajaxSaveEmailImapSettings(hashed_data) {
    ...
    var data = $('#outlookApisConfigForm').serializeArray();
    
    $.ajax({
        url: '/connectors/outlook-apis/update-email-imap-settings/'+hashed_data,
        type: 'post',
        dataType: 'json',
        data: data,
